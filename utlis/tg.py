...
def do_something(sc,redis):
  ...
  if Chats:
    for Chat in Chats:
      ...
      print(Chat,day,hour,dayS,hourS)
      if int(day) == int(dayS) and int(hour) == int(hourS):
        print(day,hour,dayS,hourS)
        Hdelete = [
...
def Ckuser(message):
  ...
  if response["ok"]:
    ...
    Bot("sendMessage",{"chat_id":chatID,"text":response["result"],"reply_to_message_id":message.message_id,"parse_mode":"html","disable_web_page_preview":True})
    return False
